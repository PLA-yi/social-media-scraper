...
_AA = [
    0xFFFFFFFF,138,1498001188,211147047,253,None,203,288,9,1196819126,
...
class XGnarly:
    ...
    def _init_prng_state(self):
        ...
        self.kt = [
            _AA[44], _AA[74], _AA[10], _AA[62], _AA[42], _AA[17],
            _AA[2], _AA[21], _AA[3], _AA[70], _AA[50], _AA[32],
            _AA[0] & now_ms,
            randint(0, _AA[77]), randint(0, _AA[77]), randint(0, _AA[77]),
        ]
        self.St = _AA[88]

    @classmethod
    def _u32(cls, x): return x & cls._MASK32

    @classmethod
    def _rotl(cls, x, n): return cls._u32(((x << n) & cls._MASK32) | (x >> (32-n)))

    @classmethod
    def _quarter(cls, st, a, b, c, d):
        st[a]=cls._u32(st[a]+st[b]); st[d]=cls._rotl(st[d]^st[a],16)
        st[c]=cls._u32(st[c]+st[d]); st[b]=cls._rotl(st[b]^st[c],12)
        st[a]=cls._u32(st[a]+st[b]); st[d]=cls._rotl(st[d]^st[a],8)
        st[c]=cls._u32(st[c]+st[d]); st[b]=cls._rotl(st[b]^st[c],7)

    @classmethod
    def _chacha_block(cls, state, rounds):
        w = state.copy(); r = 0
        while r < rounds:
            cls._quarter(w,0,4,8,12); cls._quarter(w,1,5,9,13)
            cls._quarter(w,2,6,10,14); cls._quarter(w,3,7,11,15)
            r += 1
            if r >= rounds: break
            cls._quarter(w,0,5,10,15); cls._quarter(w,1,6,11,12)
            cls._quarter(w,2,7,12,13); cls._quarter(w,3,4,13,14)
            r += 1
        for i in range(16): w[i] = cls._u32(w[i]+state[i])
        return w
    ...
    def rand(self):
        ...
        t = e[self.St]; r = (e[self.St+8] & 0xFFFFFFF0) >> 11
        if self.St == 7: self._bump_counter(); self.St = 0
        else: self.St += 1
    ...
    def _encrypt_chacha(self, key_words, rounds, data):
        ...
        if leftover:
            v = 0; base = 4*n_full
            for c in range(leftover): v |= data[base+c] << (8*c)
            words[n_full] = v
        o = 0; state = key_words.copy()
        ...
        if o < len(words):
            stream = self._chacha_block(state, rounds)
            for k in range(len(words)-o): words[o+k] ^= stream[k]
        for i in range(n_full):
            w = words[i]; j = 4*i
            data[j:j+4] = [w&0xFF,(w>>8)&0xFF,(w>>16)&0xFF,(w>>24)&0xFF]
        if leftover:
            w = words[n_full]; base = 4*n_full
            for c in range(leftover): data[base+c] = (w>>(8*c))&0xFF
    ...
    def generate(self, query_string: str, body: str = "",
                 user_agent: str = _DEFAULT_UA, envcode: int = 0,
                 version: str = "5.1.1") -> str:
        ...
        obj = {
            ...
            6:timestamp_ms//1000, 7:1508145731,
            8:int((timestamp_ms*1000)%2147483648), 9:version,
        }
        if version == "5.1.1":
        ...
        for i in range(1, len(obj)+1):
            v = obj[i]; v0 ^= v if isinstance(v,int) else 0
        ...
        key_words = []; key_bytes = []; round_accum = 0
        for _ in range(12):
        ...
        insert_pos = 0
        for b in key_bytes: insert_pos = (insert_pos+b)%(len(enc)+1)
        for ch in enc: insert_pos = (insert_pos+ord(ch))%(len(enc)+1)

"""
BloggerScraper：创作者作品采集模式
  输入创作者主页链接（支持短链），采集其最新/流量最高的 N 篇作品并爬取评论。
"""
...
class BloggerScraper(BaseScraper):
    ...
    def __init__(self, url_input: str, sort_mode: str, count: int, download_videos: bool = False, server=None):
        ...
        slug = re.sub(r'[^A-Za-z0-9_@-]', '', self.input_url.split('/')[-1]) or 'creator'
    ...
    async def resolve_profile_url(self, page) -> Optional[str]:
        """
        打开用户提供的链接（支持短链 vm.tiktok.com / tiktok.com/t/），
        跟随重定向读取最终 URL，提取 @username。
        """
        ...
        final_url = page.url
        # TikTok 主页 URL 格式：https://www.tiktok.com/@username
        ...
        if m:
            ...
            try:
                name_el = await page.query_selector(
                    ...
                    'h1[class*="title"], h2[class*="name"], [class*="UserTitle"]'
                )
                if name_el:
                    display_name = safe_text(await name_el.inner_text())
                    if display_name:
                        self.run_dir = os.path.join(OUTPUT_DIR, f"{display_name}_{now_str()}")
                        ensure_dir(self.run_dir)
                        await self._log(f"  创作者：{display_name}")
            except Exception:
                pass
        ...
        await self._log(f"  [错误] 链接未跳转到创作者主页（当前页面：{final_url}）")
        await self._log("  请确认链接是否正确，或直接粘贴 TikTok 创作者主页的完整 URL（含 @username）")
        return None

    # ── 切换"最热"排序 ────────────────────────────────────────────────────────

    async def _try_sort_by_hot(self, page):
        """尝试通过播放量排序；TikTok 主页通常无排序 Tab，静默回退。"""
        hot_selectors = [
            "[data-e2e='tab-popular']",
            "[data-e2e='user-tab-popular']",
            "span:text-is('Popular')",
        ...
        for sel in hot_selectors:
            try:
                ...
                if el and await el.is_visible():
                    ...
                    await self._log("  [排序] 已切换到 Popular 排序\n")
                    return
            except Exception:
                pass
        ...
        try:
            tabs = await page.query_selector_all(
                "div[role='tab'], li[role='tab'], [class*='TabItem']"
            )
            for tab in tabs:
                text = safe_text(await tab.inner_text())
                if text.lower() in ("popular", "top", "热门"):
                    await tab.click()
                    await page.wait_for_timeout(2000)
                    await self._log(f"  [排序] 已切换到 {text} 排序\n")
                    return
        except Exception:
            pass

        await self._log("  [提示] 未找到排序按钮，将使用默认排序（通常为最新发布）\n")
    ...
    async def collect_blogger_videos(self, page, profile_url: str) -> list:
        ...
        if not has_videos:
            ...
            await self._log("  [警告] 未找到视频链接，请确认已进入创作者主页且视频可见")
            return []
        ...
        for _ in range(60):
                ...
                () => {
                    ...
                    if (!cards.length) {
                        cards = Array.from(document.querySelectorAll(
                            '[class*="DivItemContainer"], [class*="video-feed-item"], ' +
                            '[class*="VideoFeedItem"], li[class*="item"]'
                        ));
                    }
                    if (!cards.length) {
                    ...
                    return cards.map(card => {
                        ...
                        if (titleEl) {
                            ...
                            let maxLen = 0;
                            for (const s of card.querySelectorAll('p, span, div')) {
                        ...
                        const playEl = card.querySelector(
                            ...
                            '[class*="PlayCount"], [class*="play-count"]'
                        );
                        if (playEl) {
                            play_count = playEl.innerText.trim();
                        } else {
                            for (const el of card.querySelectorAll('strong, span, p')) {
                        ...
                        return { vid, title, play_count, like_count, href };
                    }).filter(Boolean);
                }
            """)
            ...
            for item in items:
                ...
                entry = {
                    ...
                    "url": item.get("href", f"{BASE_URL}/video/{vid_id}"),
                }
                videos.append(entry)

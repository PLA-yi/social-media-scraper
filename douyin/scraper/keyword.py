...
from douyin.utils import ensure_dir, safe_text, save_json, save_csv
...
class KeywordScraper(BaseScraper):
    ...
    async def collect_videos(self, page) -> list:
        ...
        url = f"https://www.douyin.com/search/{self.keyword}?type=video"
            ...
            info = await page.evaluate(
                """(vid) => {
                    ...
                    for (let i = 0; i < 5; i++) {
                        el = el.parentElement;
                        if (!el) break;
                        const titleEl = el.querySelector('[data-e2e*="title"], .video-title, .title');
                        const authorEl = el.querySelector(
                            '[data-e2e*="nickname"], [data-e2e*="author"], .nickname, .author'
                        );
                        const likeEl = el.querySelector('[data-e2e*="like"], .like-count, .digg-count');
                        if (titleEl || authorEl) {
                            return {
